# PasteOrbit
<img width="465" height="573" alt="image" src="https://github.com/user-attachments/assets/b7b531c8-d7e5-42eb-a77e-9312c4c9c6ee" />
...
- 按类型筛选并搜索当前剪切板历史。
- 使用 Windows OCR 自动提取新复制图片中的文字，支持原文和拼音搜索。
- 在图片记录的更多菜单中将 OCR 识别文字直接粘贴到原窗口。
- 直接点击卡片粘贴原始内容。
- 将富文本粘贴为纯文本。
- 将文本或图片保存为文件。
...
图片 OCR 默认启用，可在“设置 > 常规 > 剪贴板监听”中关闭。识别使用 Windows
已安装的 OCR 语言，不会随 PasteOrbit 额外下载模型；识别文字与其他剪切板正文一样使用当前
Windows 用户凭据加密。只有启用功能后新复制的图片会自动识别。
